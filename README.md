...
| 方法 | 码率 | 画质 | 相对 PNG |
|---|---|---|---|
| **本方法（Kodak 24 张镜像平均）** | **3.728 bpp** | **51.2 dB** | **−19.0%（24/24 全部低于 PNG）** |
| PNG（无损） | 4.602 bpp | 无损 | 基线 |

> Kodak 公开测试集镜像（500×500，24 张，可复现）：近无损（51 dB）画质下平均码率低于 PNG 19.0%（24/24）；全尺寸原始影像（36.6 MB→11.7 MB）压缩比 3.1 倍。

## 核心亮点

- 🪶 **极简**：单图最小二乘预测器，绿/红/蓝通道各 5/6/7 个参数（共 18 个），随图传输；
- 🧮 **可解析**：量化失真 $E[\varepsilon^2]=q^2/12$、画质上限 $PSNR_{max}=58.92-20\lg q$ 均可解析推导并实验验证；
